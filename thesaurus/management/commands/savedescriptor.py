...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		if options['id']:
			id_descriptors = IdentifierDesc.objects.filter(pk__in=options['id']).order_by('id')
		elif options['total'] and options['from']:
			id_descriptors = IdentifierDesc.objects.filter(
				pk__in=range(options['from'], options['from'] + options['total'])).order_by('id')
		elif options['total']:
			id_descriptors = IdentifierDesc.objects.filter(pk__in=range(1, 1 + options['total'])).order_by('id')
		elif options['from']:
			id_descriptors = IdentifierDesc.objects.filter(pk__gte=options['from']).order_by('id')
		else:
			id_descriptors = IdentifierDesc.objects.all().order_by('id')
		...
		for id_descriptor in id_descriptors:
			...
			terms = TermListDesc.objects.filter(
			...
			for term in terms:
				...
				if term.record_preferred_term == 'Y' and term.concept_preferred_term == 'Y':
					...
					if term.language_code == 'en':
						...
						concept_descriptions = ConceptListDesc.objects.filter(identifier_concept=term.identifier_concept_id)
						scope_note_list = []
			...
			if pharmacological_actions:
				action_terms = TermListDesc.objects.filter(
			...
			if see_related:
				related_terms = TermListDesc.objects.filter(
			...
			allowed_qualifiers = IdentifierQualif.objects.filter(id__in=id_allowable_qualifiers).order_by('abbreviation')
			qualifier_list = []
			...
			for combination in combination_list:
				...
				combination_terms = TermListDesc.objects.filter(
			...
			try:
				descr = Descriptor.objects.get(pk=identifier_id)
				created_time = descr.created_time
