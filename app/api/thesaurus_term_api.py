...
class TermResource(Resource):
	...
	def get_search(self, request, **kwargs):
		...
		if 'words' in request.GET:
			...
			if not tree_id:
				...
				# First Level Categories Case, array with one object to fill in dehydrate
				response = [{'identifier': 2, 'term_type': "second_level"}]
			else:
	...
	def dehydrate(self, bundle):
		...
		if 'tree_id' in bundle.request.GET and bundle.obj.term_type=='first_level':
			first_level = FirstLevel.objects.filter(type='Descriptor', thesaurus=ths).order_by('treeNumber')

			flevel_list = []
			for category in first_level:
				for label in category.self_term['label']:
					if label['@language'] == lang_code:
						flevel_list.append({'term': label['@value'], 'attr': category.self_term['attr']})
			...
			decsws_response = {
				...
				'tree': {'term_list': flevel_list, 'attr': {'lang': lang}}
			}
		elif 'tree_id' in bundle.request.GET and bundle.obj.term_type == 'second_level':
			# second_level: descendants of first level categories
			tree_number = bundle.request.GET.get('tree_id').upper()
			category = FirstLevel.objects.get(treeNumber=tree_number, thesaurus=ths)

			descendants = []
			for one_descendant in category.descendant:
			...
			term_attr_list = []
			term_string = ''
			for term in category.self_term['label']:
				term_attr = {
					'descriptor': term['@value'],
					'attr': {'lang': term['@language']},
				}
			...
			decsws_response = {
				...
				'tree': {
					'self': {'term_list': [{'term': term_string, 'attr': {'tree_id': tree_number}}],
					         'attr': {'lang': lang}
					         },
					'ancestors': {},
					'preceding_sibling': {},
				...
				'record_list': {
					'record': {
						"attr": {
							"lang": lang,
							"db": 'decs',
							"mfn": category.decs_code,
						},
						'descriptor_list': term_attr_list,
						'allowable_qualifier_list': [],
						'definition': {"occ": {"attr": {"n": ""}}},
						'synonym_list': [],
						'entry_combination': [],
						'pharmacological_action_list': [],
						'see_related_list': [],
						'unique_identifier_nlm': ""
					}
				}
			}
		else:
			...
			full_term = TermFull.objects.get(pk=bundle.obj.identifier)
			...
			for term in full_term.label:
				if term['status'] == 1:
					...
					term_attr = { 'descriptor': term['@value'], 'attr': {'lang': term['@language']}}
			...
			for tree_id in tree_ids:
				...
				tree_number_id = TreeNumbersList.objects.get(tree_number=tree_id['tree_id'], identifier_id=bundle.obj.identifier)
				tree_obj = TreeFull.objects.get(id=tree_number_id.id)
