...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		if options['id']:
			id_qualifiers = IdentifierQualif.objects.filter(pk__in=options['id']).order_by('id')
		elif options['total'] and options['from']:
			id_qualifiers = IdentifierQualif.objects.filter(pk__in=range(options['from'], options['from']+options['total'])).order_by('id')
		elif options['total']:
			id_qualifiers = IdentifierQualif.objects.filter(pk__in=range(1, 1 + options['total'])).order_by('id')
		elif options['from']:
			id_qualifiers = IdentifierQualif.objects.filter(pk__gte=options['from']).order_by('id')
		else:
			id_qualifiers = IdentifierQualif.objects.all().order_by('id')
		...
		for id_qualifier in id_qualifiers:
			...
			terms = TermListQualif.objects.filter(identifier_concept__identifier_id=identifier_id).order_by('identifier_concept_id')
			label_list = []
			synonym_list = []
			concept_list = []
			active = 0
			for term in terms:
				...
				if term.record_preferred_term == 'Y' and term.concept_preferred_term == 'Y':
					...
					if term.language_code == 'en':
						...
						concept_descriptions = ConceptListQualif.objects.filter(identifier_concept=term.identifier_concept_id)
						scope_note_list = []
			...
			try:
				qualif = Qualifier.objects.get(pk=identifier_id)
				created_time = qualif.created_time
