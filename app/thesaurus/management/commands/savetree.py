  ...
  '--from --total to save a range'
	...
	def handle(self, *args, **options):
		...
		for tree_number in tree_numbers:
			...
			try:
				f_tree = FullTree.objects.get(pk=tree_number.id)
				created_time = f_tree.created_time
			except FullTree.DoesNotExist:
			...
			#save or update
			tree.save()
			self.stdout.write('Saved "%s", identifier_id "%s", ths "%s" Hierarchical Info' % (tree_id, tree_number.identifier_id, ths))
